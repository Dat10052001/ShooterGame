	...
	def shoot(self):
		if self.shoot_cooldown == 0 and self.ammo > 0:
			...
			if self.name == 'boss':
				...
				if numberRandom == 1:
					bullet1 = Bullet(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction), self.rect.centery, self.direction)
					bullet2 = Bullet(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction) +25*self.direction, self.rect.centery, self.direction)
					bullet3 = Bullet(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction) +50*self.direction, self.rect.centery, self.direction)
				else:
					bullet1 = Bullet(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction), self.rect.centery, self.direction)
					bullet2 = Bullet(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction), self.rect.centery -25, self.direction)
					bullet3 = Bullet(self.rect.centerx + (0.75 * self.rect.size[0] * self.direction), self.rect.centery +25, self.direction)
			...
			for x, tile in enumerate(row):
				if tile >= 0:
					...
					if tile >= 0 and tile <= 8:
						...
						player = Soldier('player', x * constant.TILE_SIZE, y * constant.TILE_SIZE, 1.65, 5, 50, 5)
						...
						enemy = Soldier('boss', x * constant.TILE_SIZE, y * constant.TILE_SIZE, 3.3, 9, 99, 0)
						enemy_group.add(enemy)
...
#create buttons
new_game = button.Button(constant.SCREEN_WIDTH // 2 - 125, constant.SCREEN_HEIGHT // 2 - 150, constant.new_game, 1)
about_btn = button.Button(constant.SCREEN_WIDTH // 2 - 100, constant.SCREEN_HEIGHT // 2 + 50, constant.about_img, 1)
info = button.Button(constant.SCREEN_WIDTH // 2 - 280, constant.SCREEN_HEIGHT // 2 - 200, constant.info_img, 1)
option_btn = button.Button(constant.SCREEN_WIDTH // 2 - 110, constant.SCREEN_HEIGHT // 2 - 50, constant.option_img, 1)
exit_button = button.Button(constant.SCREEN_WIDTH // 2 - 80, constant.SCREEN_HEIGHT // 2 + 150, constant.exit_img, 1)
restart_button = button.Button(constant.SCREEN_WIDTH // 2 - 130, constant.SCREEN_HEIGHT // 2 - 50, constant.restart_img, 2)
return_button = button.Button(constant.SCREEN_WIDTH // 2 - 90, constant.SCREEN_HEIGHT // 2 + 75, constant.return_img, 1)
back_btn = button.Button(constant.SCREEN_WIDTH // 2 - 350, constant.SCREEN_HEIGHT // 2 + 250, constant.back_img, 1)
...
title = button.Button(100, 0, constant.title_img, 1/2)
...
with open(f'levels/level{constant.level}_data.csv', newline='') as csvfile:
...
#show gold coins
			...
			if death_fade.fade():
				draw_text('GAME OVER', victory_font, constant.WHITE, 200, 60)
				if return_button.draw(constant.screen):
					run = False
				if restart_button.draw(constant.screen):
